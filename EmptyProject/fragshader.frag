...
void main (void){
  if( isLightOn == 0 ){
    ...
    vec3 direction2 = normalize(position2 - mypos);
    vec3 half2 = normalize(direction2 + eyedirn); 
    vec4 color2 = ComputeLight(direction2, light2_color, normal, half2, diffuse, specular, shininess) ;
        
    gl_FragColor = myColor + ambient + color0 + color1 + color2;
  }
}